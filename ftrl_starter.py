# -*- coding: utf-8 -*-
"""
Created on Tue Sep 26 22:17:02 2017
...
# A, paths
data_path = 'E:/Datasets/Kaggle/Outbrain/'
train = 'clicks_train.csv'               # path to training file
test = 'clicks_test.csv'                 # path to testing file
...
def data(path, D):
    ...
    for t, row in enumerate(DictReader(open(path))):
        ...
        # build x step1
        #对展示记录数据中的display_id,ad_id进行哈希编码
        x = []
        for key in row:
        ...
        # build x step2
        #对营销活动中ad所对应的document_id,campaign_id,advertiser_id进行哈希编码
        ad_doc_id = -1
        ...
        # build x step3
        #对广告上下文环境中ad所对应的display_id,uuid,document_id,
        #timestamp,platform,geo_location进行哈希编码
        disp_doc_id = -1
        for ind, val in enumerate(row):
            if ind==0:
                uuid_val = val
...
with open(data_path+"events.csv") as infile:
	...
	#events.next()
	next(events)
	event_header = ['uuid', 'document_id', 'platform', 'geo_location', 
                    'loc_country', 'loc_state', 'loc_dma']
	event_dict = {}
...
for e in range(epoch):
    ...
    for t, disp_id, ad_id, x, y in data(data_path+train, D):  # data is a generator
...
with open(data_path+submission, 'w') as outfile:
    ...
    for t, disp_id, ad_id, x, y in data(data_path+test, D):
        p = learner.predict(x)
        outfile.write('%s,%s,%s\n' % (disp_id, ad_id, str(p)))
        if t%1000000 == 0:
            print("Processed : ", t, datetime.now())
